 ...
 */
class GedcomEditServiceExt_20 extends GedcomEditService
{
    /** @var string[] */
    public $glevels = [];
    ...
    public function addNewFact(ServerRequestInterface $request, Tree $tree, string $fact): string
    {
        ...
        if ($DATE !== '' || $PLAC !== '' || $FACT !== '' && $FACT !== 'Y') {
            ...
            if ($PLAC !== '') {
                ...
                //[RC] added (should be __LOC for consistency but hopefully handled better in 2.1.0 anyway)
                $_LOC = $params[$fact . '_PLAC__LOC'] ?? '';
                if ($_LOC !== '') {
                    $gedrec .= "\n3 _LOC @" . $_LOC . "@";
                }
                
                $_GOV = $params[$fact . '__GOV'] ?? '';
                if ($_GOV !== '') {
                    $gedrec .= "\n3 _GOV " . $_GOV;
                }
                //[RC] added end
